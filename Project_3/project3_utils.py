...
Core data utilities for Project 3: Correlation, Regression, and Hypothesis Testing.

Contains only the three functions used in Session 1:
  - to_baostock_code : convert 6-digit codes to baostock format
  - get_stock_data   : pull daily OHLCV from baostock for one stock
  - load_or_fetch    : cache-aware wrapper around get_stock_data

Scope is intentionally narrow. By the rule of three, add a helper here only
after writing it inline in two separate notebooks and being about to write
it a third time. Risk metrics, plotting helpers, and limit-hit detection
belong in their own modules when needed, not here.
"""
